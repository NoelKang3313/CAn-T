...
public class Room1DoorController : MonoBehaviour
{
    ...
    void RotateDoor()
    {
        if (GameManager.instance.isRoomDoorOpen)
        {
            ...
            if (gameObject.transform.rotation.y <= 0.7f)
            {
                ...
                GameManager.instance.getRoomKey = false;
                GameManager.instance.isRoomDoorOpen = false;
                GameManager.instance.enterNextRoom = true;
            }
        }
    }
    ...
    private void OnTriggerStay(Collider other)
    {
        if (other.gameObject.CompareTag("Player"))
        {
            if (GameManager.instance.activateUI)
            {
                ...
                if (Input.GetKey(KeyCode.F) && !GameManager.instance.getRoomKey)
                {
                ...
                else if (Input.GetKey(KeyCode.F) && GameManager.instance.getRoomKey)
                {
                    GameManager.instance.isRoomDoorOpen = true;
                }
            }
            else
            {
